...
export class DBPartieSimple {
    ...
    private getImageDiffAsArrays(partie: PartieSimpleInterface): void {
        const imageMod: string = p.resolve(constantes.IMAGES_DIRECTORY + "/image3.bmp.txt");
    ...
    private async genererImageMod(partie: PartieSimpleInterface): Promise<void> {
        ...
        const pyScript: string = p.resolve(constantes.BMPIFF_PATH);
        const imageOri1: string = p.resolve(constantes.IMAGES_DIRECTORY + "/image1.bmp");
        const imageOri2: string = p.resolve(constantes.IMAGES_DIRECTORY + "/image2.bmp");
        const imageMod: string = p.resolve(constantes.IMAGES_DIRECTORY + "/image3.bmp");
